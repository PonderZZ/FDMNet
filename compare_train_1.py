...
from FocalTransNet import Net as FocalTransNet
from data_1 import ACDC_transforms, AcdcDataset2D, ComposeTransforms


# ==========================================
# 1. Loss Functions (2D Adapted)
# ==========================================
...
def main():
    ...
    # 在这里切换你想对比的模型名字
    MODEL_TYPE = 'FocalTransNet'  # 候选: 'HDC_Net', 'FocalTransNet', 'UNet'

    DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    TRAIN_DATA_ROOT = '/home/zgm/ZHF/HDC/ACDC/ACDC_2DData/train'
    VAL_DATA_ROOT = '/home/zgm/ZHF/HDC/ACDC/ACDC_2DData/test'
    OUT_DIR = f"./runs/{DATASET}_{MODEL_TYPE}_{time.strftime('%Y%m%d-%H%M')}"
    ...
    if MODEL_TYPE == 'HDC_Net':
        model = HDC_Net(in_channels=1, out_channels=4, base_c=32).to(DEVICE)
    elif MODEL_TYPE == 'FocalTransNet':
        model = FocalTransNet(img_size=TARGET_SIZE[0], dim_in=1, dim_out=4, device=str(DEVICE)).to(DEVICE)
    elif MODEL_TYPE == 'UNet':
        model = UNet(spatial_dims=2, in_channels=1, out_channels=4,
    ...
    # --- Loop ---
    best_dice = 0.0
    best_dice_per_class = {1: 0.0, 2: 0.0, 3: 0.0}  # 新增：用于存储最佳指标
    ...
    logger.info(f"Best Validation mDice: {best_dice:.4f}")
    logger.info(f"   - RV  (Class 1): {best_dice_per_class.get(1, 0.0):.4f}")
    logger.info(f"   - MYO (Class 2): {best_dice_per_class.get(2, 0.0):.4f}")
    logger.info(f"   - LV  (Class 3): {best_dice_per_class.get(3, 0.0):.4f}")
    logger.info("=" * 50)
